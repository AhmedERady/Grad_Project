...
architecture rtl of interface_level is
  ...
  begin
    	...
    	output_ram 	: out_ram 	port map (clk, rst, wr, outputrow, outputcol, res_out, res_in);
    	kernel_ram 	: ker_ram 		port map (clk, kernel_wr, kernelrow, kernelcol, kernel_datain, mat1);
    	input_ram 	: inp_ram	 	port map (clk, input_wr, inputrow, inputcol, input_datain, mat2);
	kernel_row	: mux		port map (kernel_en, i_out, kernel_row_adr, kernelrow);
	kernel_col	: mux		port map (kernel_en, k_out, kernel_col_adr, kernelcol);
	input_row	: mux		port map (input_en, k_out, input_row_adr, inputrow);
	input_col	: mux		port map (input_en, j_out, input_col_adr, inputcol);
	output_row	: mux		port map (output_en, i_out, output_row_adr, outputrow);
	output_col	: mux		port map (output_en, j_out, output_col_adr, outputcol);
